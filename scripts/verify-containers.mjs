...
const projectName = `venue-platform-phase10-${process.pid}`;
...
try {
  ...
  process.stderr.write(`Phase 10 container verification cannot start: ${message}\n`);
  process.exit(1);
}
...
try {
  ...
  logStep(5, 'Running the real Phase 1 through Phase 10 API integration suites');
  ...
  for (const application of ['web', 'api', 'worker']) {
    docker(
      ...
      '--tag',
      `venue-${application}:phase10-verification`,
      '.',
    );
  }

  process.stdout.write('\nPhase 10 container verification passed.\n');
} finally {
